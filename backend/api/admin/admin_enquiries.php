...
try {
    ...
    if ($method === 'POST') {
        ...
        $status = in_array(($data['status'] ?? 'new'), ['new', 'contacted', 'closed'], true) ? $data['status'] : 'new';
        $stmt = db()->prepare(
        ...
        send_json(['message' => 'Enquiry created.', 'id' => (int) db()->lastInsertId()], 201);
    }

    if ($method === 'PUT') {
        $id = (int) ($_GET['id'] ?? 0);
        if ($id <= 0) send_json(['message' => 'Missing enquiry id.'], 422);
        $data = read_json();
        require_fields($data, ['name', 'email', 'message']);
        $status = in_array(($data['status'] ?? 'new'), ['new', 'contacted', 'closed'], true) ? $data['status'] : 'new';
        $stmt = db()->prepare(
        ...
        send_json(['message' => 'Enquiry updated.']);
    }
    ...
    if ($method === 'DELETE') {
        ...
        send_json(['message' => 'Enquiry deleted.']);
    }
